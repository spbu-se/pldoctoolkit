...
public class ReplaceWithNestAction extends Action implements
	...
	public void run() {
		...
		InputDialog dialog = new InputDialog(editor.getSite().getShell(),
				"Create nest...",
				"Replace selected fragment with nest.\nEnter id of new Nest",
				CreateNest.getAppropriateNestId("", projectContent), new IInputValidator() {
					@Override
					public String isValid(String newText) {
						for (LangElem nest : projectContent.nests) {
							if (nest.attrs.getValue(LangElem.ID)
									.equals(newText))
