...
public class ReplaceWithNest {
	...
	public void perform() {
		...
		if (isValide) {
			ArrayList<Element> removedChilds = firstPartOfPerform();
	...
	public ArrayList<Element> firstPartOfPerform() {
		...
		return from.parent.removeChilds(fromIdx, toIdx);
	}
	...
	private LangElem createNewNest() {
		LangElem entry = new LangElem(LangElem.NEST, prefex + LangElem.NEST,
				null, parent, parent.getDRLDocument(), new AttributesImpl());
		((AttributesImpl) entry.attrs).addAttribute(LangElem.ID, LangElem.ID,
				LangElem.ID, "", nestId);
		return entry;
	}
}