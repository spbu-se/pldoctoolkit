...
public abstract class Specifier<T extends IPartOfInfEl> {
...
//	}
	...
	public List<T> specifyPart4XML(T partOfIE) {
		List<T> rez = specifyPart4XMLPrivate(partOfIE);
		deleteEmptyParts(rez);
		return rez;
	}
	
	private void deleteEmptyParts(List<T> rez) {
		for (Iterator<T> iterator = rez.iterator(); iterator.hasNext();) {
			T t = iterator.next();
	...
	private List<T> specifyPart4XMLPrivate(T partOfIE) {
		tags.clear();
		System.out.println("\nBEGIN start:"+partOfIE.getStartPos4EntireDocument()
				+" end:"+ partOfIE.getEndPos4EntireDocument()+ " :\n" + partOfIE.getText() + "\n:END");
		String text = partOfIE.getText();
...
//			rez.add(text.substring(indexOfFirstLT+1));
			PositionInText startPos = addAbsoluteIndexToPosition(
					partOfIE.getStartPos4EntireDocument(), indexOfFirstLT+1, text);
			PositionInText endPos = partOfIE.getEndPos4EntireDocument();
			T inst = createPartOfInfEl(
					partOfIE.getInfEl(), startPos, endPos);
			rez.add(inst);
			return rez;
		}
		if (indexOfFirstLT == -1){
...
//			rez.add(text.substring(0, indexOfFirstGT));
			PositionInText startPos = partOfIE.getStartPos4EntireDocument();
		...
		int prefixIndex = addFirstOpeningTagToTagsAndGetPrefixIndex(partOfIE);
		if (prefixIndex == -1){
			...
			while(prefixIndex != -1){
				prefixIndex = addNextTagToTagsAndGetPrefixIndex(partOfIE, prefixIndex);
			}
			...
			for (PosPara para : paras) {
				...
				T inst = createPartOfInfEl(partOfIE.getInfEl(), startPos, endPos);
				rez.add(inst);				
			}
			return rez;
		}	
	...
	protected abstract T createPartOfInfEl(LangElem infEl, PositionInText startPos, PositionInText endPos) ;
	...
	private int addNextTagToTagsAndGetPrefixIndex(T clone,
			int prefixIndex) {
	...
	private int addFirstOpeningTagToTagsAndGetPrefixIndex(T clone) {
