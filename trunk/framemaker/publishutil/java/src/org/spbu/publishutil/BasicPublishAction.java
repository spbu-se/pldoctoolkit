...
public class BasicPublishAction {
	...
	static {
		DRL2DOCBOOK_FILE = new File(PublishUtil.getRunningAppPath() + "/xsl/drl/drl2docbook.xsl");
		DOCBOOK2HTML_FILE = new File(PublishUtil.getRunningAppPath() + "/xsl/docbook/html/docbook.xsl");
		DOCBOOK_SCHEMA_FILE = new File(PublishUtil.getRunningAppPath() + "/schema/docbook/docbook.rng");
	}
	...
	public BasicPublishAction(File destinationFile, File docbook2formatFile, Boolean bTransform2DBOnly) throws Exception {
	...
	public void run(File file, String fipIdExt) throws RuntimeException {
		try {
			...
			fipId = null;
			if (fipList.isEmpty())
				throw new RuntimeException("Couldn't find any final inf product in specified file");
			...
			if (fipId == null)
				throw new RuntimeException("Couldn't select any final inf product");
	...
	private void processXPathException(XPathException xpathError, int severity) throws XPathException {
		...
		if (errorObject instanceof SingletonNode) {
			...
			// ignore
			// these exceptions point to internal file, but provoked by incorrect
			// user files, which is catched elsewhere, so there is no reason to show these ones to the user 
