...
import plweb.diagram.util.SourceRefElementContainer;
...
public class VariativitySelectionPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		modelViewer = new CheckboxTreeViewer(
		...
		modelViewer.setContentProvider(new AdapterFactoryContentProvider(
				PlwebDiagramEditorPlugin.getInstance().getItemProvidersAdapterFactory()));
			...
			public String getText(Object object) {
				if (object != null && object instanceof Element) {
					...
					if (object instanceof Area) {
						...
						className = "Page";
					} else if (object instanceof plweb.Node) {
						className = "Node";
					}
					...
					if (element instanceof TargetRefElement) {
						...
						if (!modelViewer.getChecked(parent)) {
							...
							if (parentGroup.getType().equals(GroupType.XOR)) {
								TargetRefElement firstElement = parentGroup.getClass_().get(0);
	...
	protected boolean validatePage() {
		...
		try {
			ProjectWebRatio projectWr = new ProjectWebRatio(diagramRoot.getProjectPath());
			Map<String, String> missingPages = projectWr.getMissingPages(checkedPages);
			if (!missingPages.isEmpty()) {
				String reqPage = (String) missingPages.keySet().toArray()[0];
				String byPage = missingPages.get(reqPage);
				setMessage("Page '" + reqPage + "' is required by '" + byPage + "' page", IMessageProvider.WARNING);
			} else {
				setMessage(null);
			}
			...
			if (!container.isValid(checkedPages)) {
				...
				if (!invalidElements.isEmpty()) {
					...
					if (element.getTitle() != null) {
						errorMessage += ": node '" + element.getTitle() + "' is invalid";
					} else {
