...
public class PlwebUpdateWRProjectAction implements IObjectActionDelegate {
		...
		try {
			...
			if (diagramRoot.getType().equals(DiagramType.PRODUCT)) {
				MessageDialog.openError(
						getShell(),
						"Not supported",
						"This action is not supported for products"
				);
			} else {
			...
			ErrorDialog.openError(getShell(), "Error", null, 
					new Status(IStatus.ERROR, PlwebDiagramEditorPlugin.ID, "Error happened while updating PLWeb project"));
		} catch (Exception e) {
			e.printStackTrace();
		}
