...
def combine_groups_n_ext_with_int_tree_future(available_groups: "list[clones.CloneGroup]"):
    """
    Algorithm:
    AG = available_groups
    AVG = available_variative_groups
    ...
    while cycle:
        ...
        # (2)
        smallest_bvc = clones.bassett_variativity_threshold
        best_ve = None
        best_g1_g2 = None
        for g1, g2 in tojoin:
            logging.debug("AVG %d ->" % (len(avg),))
            ...
            if basset_variativity_coefficient <= smallest_bvc:
                cycle = True  # check for (3)
                best_g1_g2 = g1, g2
                best_ve = new_ve
        ...
        if cycle:
            # (2.1)
            avg.remove(best_g1_g2[0])
            avg.remove(best_g1_g2[1])
            # (2.3)
            avg.add(best_ve)
            logging.debug("AVG %d <-" % (len(avg),))
        else:
