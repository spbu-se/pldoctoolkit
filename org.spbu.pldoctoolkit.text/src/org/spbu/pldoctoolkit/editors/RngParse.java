...
public class RngParse {
	...
	public RngParse(URL[] shm) {
		...
		for (int i = 0; i < shm.length; i++) {
				parser.parse(new InputSource(shm[i].toString()), new SaxParserSchem());
		...
		for (Iterator iterator = defines.values().iterator(); iterator
			...
			if ((!vertex.child.isEmpty()) && (vertex.child.getFirst().type == 10)){
	...
	//  find out if  we can reack element-tag from this vertex
...
//		Vertix j = (Vertix) it.next();
//		while ((it.hasNext()) && ((j.type == 8) || ((j.type == 6)))) {
//			j = (Vertix) it.next();
...
//		}
		...
		for (int i = 0; i <= l.size(); i++) {
			...
			while (!q.isEmpty()) {
				...
				if (w.at.type == 9) {
					...
					if (notAllowed.contains(defines.get(w.at.attr))){continue;}
	...
	public HashSet<String> findOptionalAttrs(Vertex v, LinkedList<String> attrs) {
		LinkedList<WayTo> q = new LinkedList<WayTo>();
		HashSet<Integer> nodeal = new HashSet<Integer>();
		HashSet<String> res = new HashSet<String>();
		nodeal.add(2);
