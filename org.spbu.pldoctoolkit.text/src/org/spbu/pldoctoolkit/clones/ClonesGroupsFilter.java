...
final class ClonesGroupsFilter {
	...
	private static final Specifier<ICloneInst> clonesSpecifier = new Specifier<ICloneInst>(){

		@Override
		protected ICloneInst createPartOfInfEl(LangElem infEl,
				PositionInText startPos, PositionInText endPos) {
			return CloneInstImpl.createCloneInstByPositions4EntireDoc(infEl, startPos, endPos);
		}};
			...
			if (groupContainsOnlyExactClones(clonesGroup)){
				if (groupTextIsSimpleText(clonesGroup)){
					...
					for (ICloneInst clone : clonesGroup.getInstances()) {
						List<ICloneInst> specifiedClones = clonesSpecifier.specifyPart4XML(clone);
						if (firstOperation){
...
//			String text = clonesGroup.getInstances().get(0).getText()
//					.replace("\t", "").replace("\n", "").replace(" ", "");
//			if(text.length() == 0){
//				iterator.remove();
//			}else{
...
//	}
	...
	private boolean groupContainsOnlyExactClonesWithoutRegister(
		...
		for (ICloneInst clone : clonesGroup.getInstances()) {
			if (template == null){
				template = clone.getText().toLowerCase();
			}else{
				if (!textOfClonesEquals(template, clone.getText().toLowerCase())){
					return false;
				}
			}
		}
	...
	private boolean groupContainsOnlyExactClones(IClonesGroup clonesGroup) {
		...
		for (ICloneInst clone : clonesGroup.getInstances()) {
			if (template == null){
				template = clone.getText();
			}else{
				if (!textOfClonesEquals(template, clone.getText())){
					return false;
				}
			}
		}
