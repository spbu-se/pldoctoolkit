...
public class SplitInfElem {
	...
	private void init() {
		...
		PositionInText lastSonEndPos = targetElem.getChilds().get(
				targetElem.getChilds().size()-1).getEndPos();
		
		firstRefactor.setValidationPararams(project, doc, firstSonStartPos,
				new PositionInText(splitPosition.line, splitPosition.column));
		secondRefactor.setValidationPararams(project, doc, new PositionInText(
				splitPosition.line, splitPosition.column), lastSonEndPos);
		firstRefactor.reset();
	...
	public void setValidationParams(ProjectContent projectContent,
			DRLDocument ldoc, PositionInText pos1) {
	...
	public void setParams(ProjectContent projectContent,
			DRLDocument ldoc, IEditorPart editor, PositionInText splitPosition) {
		this.project = projectContent;
		this.doc = ldoc;
	...
	public void perform() {
		...
		// 2 create first part
		LangElem firstInfElem = createNewInfElem(parent, firstRefactor.elemId,
				firstRefactor.elemName);
		...
		LangElem secondInfElem = createNewInfElem(parent,
				secondRefactor.elemId, secondRefactor.elemName);
		...
		for (LangElem infElemRef : getInfElemRefs()) {
			...
			if (optionalIdx != -1) {
				isOptional = infElemRef.attrs.getValue(optionalIdx)
						.toLowerCase().equals(LangElem.TRUE);
			}
			...
			LangElem firstRef = createNewInfElemRef(refParent,
					firstRefactor.refId, firstRefactor.elemId);
			refParent.getChilds().add(refIdx, firstRef);
			// 4.3 create second ref
			LangElem secondRef = createNewInfElemRef(refParent,
					secondRefactor.refId, secondRefactor.elemId);
			Util.addNewLine(refParent, refIdx + 1);
			refParent.getChilds().add(refIdx + 2, secondRef);

			if (isOptional) {
				((AttributesImpl) firstRef.attrs).addAttribute(
						LangElem.OPTIONAL, LangElem.OPTIONAL,
						LangElem.OPTIONAL, LangElem.OPTIONAL, LangElem.TRUE);
				((AttributesImpl) secondRef.attrs).addAttribute(
						LangElem.OPTIONAL, LangElem.OPTIONAL,
						LangElem.OPTIONAL, LangElem.OPTIONAL, LangElem.TRUE);
			}
			...
			for (LangElem adapter : project.adapters) {
				if (adapter.attrs.getValue(LangElem.INFELEMREFID).equals(
						elemRefIdToFind)) {
					...
					for (Element elem : adapter.getChilds()) {
						if (elem instanceof LangElem) {
							...
							String nestid = nestRef.attrs
									.getValue(LangElem.NESTID);
					...
					LangElem newAdapter = createNewAdapter(adapterParent,
							firstRefactor.refId);
					...
					newAdapter = createNewAdapter(adapterParent,
							secondRefactor.refId);
	...
	private LangElem createNewAdapter(LangElem parent, String infElemRefId) {
		LangElem newAdapter = new LangElem("Adapter", prefex + "Adapter", null,
				parent, parent.getDRLDocument(), new AttributesImpl());
