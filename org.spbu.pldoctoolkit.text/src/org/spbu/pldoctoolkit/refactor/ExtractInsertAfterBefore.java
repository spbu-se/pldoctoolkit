...
public class ExtractInsertAfterBefore {
	...
	public void setValidationPararams(DRLDocument doc, PositionInText fromText,
			PositionInText toText, int type) {
	...
	private boolean isNest(Element elem) {
		return (elem instanceof LangElem)
				&& ((LangElem) elem).tag.equals(LangElem.NEST);
	}
	...
	public void perform() {
		...
		if (isValide) {
			...
			getInfElemRefs();
			ArrayList<Element> childsToInsert = from.parent.removeChilds(
					fromIdx, toIdx);
			...
			for (LangElem infElemRef : infElemRefs) {
				...
				for (LangElem adapter : project.adapters) {
					if (adapter.attrs.getValue(LangElem.INFELEMREFID).equals(
							elemRefIdToFind)) {
						LangElem insertSmth = createInsertAfterBefore(nestId,
								adapter);
	...
	private LangElem createSmthWithNestID(String tag, String nestId,
			LangElem parent) {
		LangElem newSmth = new LangElem(tag, prefex + tag, null, parent, parent
				.getDRLDocument(), new AttributesImpl());
		((AttributesImpl) newSmth.attrs).addAttribute(LangElem.NESTID,
				LangElem.NESTID, LangElem.NESTID, "", nestId);
		return newSmth;
	}
}
