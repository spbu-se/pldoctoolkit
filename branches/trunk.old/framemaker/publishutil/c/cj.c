...
#define CJ_PROXY_EXEC_METHOD 1

static cjMethod_t proxyMethods[] =
{
   {"<init>", "()V", NULL},
   {"execute", "(Ljava/lang/Object;)Ljava/lang/Object;", NULL},
};
...
int cjProxyExec(cjObject_t *pProxy, jobject data, jobject *pOutData)
{
   return callProxyMethod(pProxy, data, pOutData, CJ_PROXY_EXEC_METHOD);
}
...
int cjProxyExecString(cjObject_t *pProxy, char *inData, char *outData)
{
   long jret = 0;
   int rc = CJ_ERR_SUCCESS;
   cjClass_t *pClass = pProxy->clazz;
   ...
   if (rc == CJ_ERR_SUCCESS)
   {
      rc = cjProxyExec(pProxy, instring, &outobject);
   }
